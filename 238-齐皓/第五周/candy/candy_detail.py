...
dx, dy = img.shape
img_new = np.zeros(img.shape)  # 存平滑后的图像，zeros得到的是浮点型数
tmp = dim//2
...
plt.figure(1)
plt.imshow(img_new.astype(np.uint8), cmap='gray')  # img_new是255的浮点型数据，强制类型转换，gray灰阶
plt.axis('off')
